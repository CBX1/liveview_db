defmodule PokemonDbWeb.SearchLive do
  ...
  def handle_event(
        ...
        set = from( p in Pokemon, as: :pkmn, join: m in Move, as: :move, on: m.pokemon_id == p.id, select: %{id: p.id, name: p.name, description: p.description, type1: p.type1, type2: p.type2, hidden_ability: p.hidden_ability, regular_abilities: p.regular_abilities, evolution: p.evolution, growth_rate: p.growth_rate,  moves: fragment("array_agg(?)",m)}, group_by: p.id, where: ilike(p.name,^"%#{pokemon_name}%") and (ilike(p.type1,^"%#{pokemon_name}%")  or ilike(p.name,^"%#{pokemon_name}%"))     )
        sp = from(p in BaseStat, join: s in subquery(set), on: s.id == p.pokemon_id, select: %{id: s.id, name: s.name,description: s.description, type1: s.type1, type2: s.type2, hidden_ability: s.hidden_ability, regular_abilities: s.regular_abilities, evolution: s.evolution, growth_rate: s.growth_rate, moves: s.moves, atk: p."ATK", def: p."DEF", speed: p."SPEED", spd: p."Sp. DEF", spa: p."Sp. ATK", hp: p."HP" })
    ...
    def handle_event("pokemon_new", %{"pokemon_form" => params}, socket) do
        ...
        sp = from(p in BaseStat, join: s in subquery(set), on: s.id == p.pokemon_id, select: %{id: s.id, name: s.name,description: s.description, type1: s.type1, type2: s.type2, hidden_ability: s.hidden_ability, regular_abilities: s.regular_abilities, evolution: s.evolution, growth_rate: s.growth_rate, moves: s.moves, atk: p."ATK", def: p."DEF", speed: p."SPEED", spd: p."Sp. DEF", spa: p."Sp. ATK", hp: p."HP" }, where: (ilike(s.hidden_ability, ^"%#{params["ability"]}%") or fragment("?=Any(regular_abilities)", ^"#{params["ability"]}") or ^params["ability"] == "") )
         mm = from(pl in PokemonLocation, join: l in Location, on: l.id == pl.location_id, right_join: s in subquery(sp), on: s.id == pl.pokemon_id, select: %{type1: s.type1, name: s.name, type2: s.type2, id: s.id, l: fragment("array_agg(?)", l.name)}, group_by: [s.id, s.name, s.description, s.type1, s.type2, s.hidden_ability, s.regular_abilities, s.evolution, s.growth_rate, s.moves, s.atk, s.def, s.speed, s.spd, s.spa, s.hp], where: ilike(l.name,^"%#{params["location"]}%") or ^params["location"] == "", limit: 50)
        pokemons = Repo.all(mm)
        socket =
            socket
